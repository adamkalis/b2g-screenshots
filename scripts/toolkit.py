...
def screenshot_changed(screenshot_name):
  ...
  old = Image.open(screenshot_name + '.old')
  diff = ImageChops.difference(new, old)
  if diff.getbbox:
    diff.save(screenshot_name + '.diff', 'JPEG')
    return True
  else:
    return False
